...
function getCarInfoByIndex(inventory, index) {
  ...
  return `The car is a ${aCar.car_make} ${aCar.car_model}`;
}
...
function sortCarInventory(inventory) {
  ...
  //     if (inventory[e].car_model === carModels[i]) {
  //       sortedInventory.push(inventory[e]);
  //     }
  //   }
  // }
 ...
 */
function getOlderCars(inventory, aYear) {
  const carsByYear = [];

  for (let i = 0; i < inventory.length; i++) {
    if (inventory[i].car_year <= aYear) {
      carsByYear.push(inventory[i].car_year);
    }
  }
  return carsByYear;
}
